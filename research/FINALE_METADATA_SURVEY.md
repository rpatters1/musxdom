...
5. `SMInstrumentsMap.json` and MusicXML `sounds.xml` connect Finale instrument UUIDs to Standard Music Font Layout/MusicXML instrument-sound identifiers. The bundled `sounds.xml` also explicitly classifies UUIDs as solo or ensemble instruments.
...
| `Finale.app/Contents/PlugIns/MusicXML.bundle/Contents/Resources/DTD/` | `sounds.xml` | MusicXML instrument-sound taxonomy with Finale UUIDs embedded in explicit `<solo>` and `<ensemble>` elements. The Finale 27 snapshot has 736 unique solo UUIDs and 25 unique ensemble UUIDs with no overlap. Of the 502 unique UUIDs in `FinaleInstUUIDs.xml`, it classifies 411 as solo and 17 as ensemble, leaving 74 unclassified. | **Preserve.** This is the clearest surveyed source for determining whether an `instUuid` denotes a solo or ensemble instrument. Consumers must retain an unknown state rather than treating every UUID absent from the ensemble set as solo. `GIFF.soundmap` corroborates many assignments but is less authoritative because one instrument UUID may offer solo, player, and section playback choices. |
...
- `Finale.app/.../MusicXML.bundle/.../DTD/`: MusicXML DTD/XSD/module/entity files, version-conversion XSLT, `catalog.xml`, and `sounds.xml`; plus the bundled Java runtime's configuration, licenses, security policies, and properties. Most schemas and transforms are upstream MusicXML resources, but `sounds.xml` is also Finale-specific interoperability metadata because its `<solo>` and `<ensemble>` elements contain Finale instrument UUIDs. The duplicated Java runtime trees and legal/config files are not Finale metadata.
...
1. `FinaleInstUUIDs.xml`, `SMInstrumentsMap.json`, the current MusicXML `sounds.xml`, and `dictionary.instruments.xml`.
...
7. Older-only `instrument.txt`, `GIFFinstruments.txt`, `Tapspaceinstruments.txt`, `chordmap.txt`, `gmsounds.xml`, and historical `sounds.xml` revisions.
